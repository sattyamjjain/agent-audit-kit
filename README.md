...
<p align="center">
  ...
  <a href="#what-it-scans"><img src="https://img.shields.io/badge/rules-341-blue.svg" alt="Rules: 341"></a>
...
- **<!-- rule-count:total -->341<!-- /rule-count --> rules** across 14 security categories, covering the 2026 CVE wave
...
- **Remediation guidance: all <!-- rule-count:total -->341<!-- /rule-count --> of <!-- rule-count:total -->341<!-- /rule-count --> rules.** Every finding tells you what to change and why — that is the product, and it has no gap.
- **Mechanical fix recipes: <!-- fix-recipe-coverage:count -->11<!-- /fix-recipe-coverage --> of <!-- rule-count:total -->341<!-- /rule-count --> rules (<!-- fix-recipe-coverage:pct -->3.2<!-- /fix-recipe-coverage -->%).** This is a scope decision, not coverage: **a recipe ships only where the remediation is deterministic and one-line** — exactly one correct edit, confirmable from the diff. Everything else stays advisory on purpose, because a fix that needs judgement is a fix that can be wrong silently. `agent-audit-kit fix` applies these; the narrower `suggest --auto-pr` allow-list is the subset AAK will open a PR for.
...
**<!-- rule-count:total -->341<!-- /rule-count --> rules total.** Every finding includes severity, evidence, remediation, OWASP references, Adversa references, and CVE links where applicable.
...
| Detection rules (static) | **<!-- rule-count:total -->341<!-- /rule-count -->** | Runtime policies, not rules | ~30 | LLM-assisted |
...
cd agent-audit-kit
pip install -e ".[dev]"
pytest -v                          # <!-- test-count:total -->2,287<!-- /test-count --> test functions
ruff check .                       # Lint
