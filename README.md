...
<p align="center">
  ...
  <a href="#what-it-scans"><img src="https://img.shields.io/badge/rules-190-blue.svg" alt="Rules: 190"></a>
...
- **<!-- rule-count:total -->190<!-- /rule-count --> rules** across 11 security categories, covering the 2026 CVE wave
- **<!-- scanner-count:total -->57<!-- /scanner-count --> scanner modules** including AST-based Python taint analysis and regex pattern scanners for TypeScript/JavaScript and Rust
...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: sattyamjjain/agent-audit-kit@v0.3.12
        with:
          fail-on: high
```
...
**<!-- rule-count:total -->190<!-- /rule-count --> rules total.** Every finding includes severity, evidence, remediation, OWASP references, Adversa references, and CVE links where applicable.
...
| Detection rules (static) | **<!-- rule-count:total -->190<!-- /rule-count -->** | Runtime policies, not rules | ~30 | LLM-assisted |
