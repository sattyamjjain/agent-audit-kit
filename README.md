...
<p align="center">
  ...
  <a href="#what-it-scans"><img src="https://img.shields.io/badge/rules-217-blue.svg" alt="Rules: 217"></a>
...
- **<!-- rule-count:total -->217<!-- /rule-count --> rules** across 11 security categories, covering the 2026 CVE wave
- **<!-- scanner-count:total -->71<!-- /scanner-count --> scanner modules** including AST-based Python taint analysis and regex pattern scanners for TypeScript/JavaScript and Rust
...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: sattyamjjain/agent-audit-kit@v0.3.29
        with:
          fail-on: high
```
...
**<!-- rule-count:total -->217<!-- /rule-count --> rules total.** Every finding includes severity, evidence, remediation, OWASP references, Adversa references, and CVE links where applicable.
...
| **ASI02** | Tool Misuse | 34 |
| **ASI03** | Memory Poisoning | 51 |
| **ASI04** | Identity & Privilege Abuse | 36 |
...
| Detection rules (static) | **<!-- rule-count:total -->217<!-- /rule-count -->** | Runtime policies, not rules | ~30 | LLM-assisted |
