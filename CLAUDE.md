...
- **351 rules** across 14 security categories
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 351 RuleDefinition entries (rule registry)
...
- **Rule registry**: `rules/builtin.py` defines all 351 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
