...
- **294 rules** across 12 security categories
- **88 scanner modules** including AST-based Python taint analysis plus regex dangerous-sink pattern scanners for TypeScript/JavaScript and Rust (pattern matching, not taint flow)
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 294 RuleDefinition entries (rule registry)
  scanners/            # 88 scanner modules (core set shown), each exports scan() -> (list[Finding], set[str])
...
- **Rule registry**: `rules/builtin.py` defines all 294 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
