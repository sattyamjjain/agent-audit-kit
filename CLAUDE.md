...
- **303 rules** across 12 security categories
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 303 RuleDefinition entries (rule registry)
...
- **Rule registry**: `rules/builtin.py` defines all 303 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
