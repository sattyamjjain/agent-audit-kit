...
- **300 rules** across 12 security categories
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 300 RuleDefinition entries (rule registry)
...
- **Rule registry**: `rules/builtin.py` defines all 300 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
