...
- **339 rules** across 14 security categories
- **100 scanner modules** including AST-based Python taint analysis plus regex dangerous-sink pattern scanners for TypeScript/JavaScript and Rust (pattern matching, not taint flow)
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 339 RuleDefinition entries (rule registry)
  scanners/            # 100 registered scanners (102 .py files on disk — the registry is authoritative)
...
- **Rule registry**: `rules/builtin.py` defines all 339 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
